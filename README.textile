...
I like the comfort of *static typing*, so, inspired from "Blaze HTML":http://jaspervdj.be/blaze/, Glitter is a proof of concept of an *embedded Scala DSL* allowing to write HTML templates with a "haml":http://www.haml-lang.com like syntax, but directly *in your Scala code*. ("Read more on my motivations…":glitter/wiki/Motivation)

You can see "my personal website":https://github.com/julienrf/julienrf/tree/master/src/main/scala/julienrf as an example of website entirely written in Scala with Glitter.
...
You can use it in your sbt project by adding this line (adapt it to the latest release version) :

bc. val glitter = "glitter" % "glitter" % "0.1" from "file:///where/you/downloaded/glitter-0.1.jar"
...
Glitter is defined as a "sbt":http://code.google.com/p/simple-build-tool/ project. You can run the tests using the @sbt test@ command, or run the integrated bigtable benchmark with @sbt run@.

h2. Syntax

To use Glitter you first need to import the definitions of the @glitter@ package object:
...
To iterate through a collection you can use the @forM[A](elmts: Traversable[A])(bind: A => Xml): Xml@ control structure provided by Glitter:

bc. def listArticles(articles: List[Article]): Xml =
  'ul :: forM (articles) ('li :: _.name)

The @forM@ function takes an @Traversable[A]@ and a function of type @A => Xml@ as parameters. It produces a sequence of xml content using the @|@ operator between all the collection elements after having bound each one with the @bind@ function.
