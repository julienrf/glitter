...
I like the comfort of *static typing*, so, inspired from "Blaze HTML":http://jaspervdj.be/blaze/, Glitter is a proof of concept of an *embedded Scala DSL* allowing to write HTML templates with a "haml":http://www.haml-lang.com like syntax, but directly *in your Scala code*. "Read more on my motivations":julienrf/glitter/wiki/Motivation
...
The @forM@ function takes an @Iterable[A]@ and a function of type @A => Xml@ as parameters. It produces a sequence of xml content using the @|@ operator between all the collection elements after having bound each one with the @bind@ function.
